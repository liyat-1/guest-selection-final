...
export function renumber(steps: SequenceStep[]): SequenceStep[] {
  let n = 0;
  return steps.map((s) => (s.kind === "initial" ? s : { ...s, name: `Follow-up ${++n}` }));
}
...
export function channelState(step: SequenceStep, key: ChannelKey, active: boolean): ConfigState {
...
export type RuleTrigger =
  "booked" | "opened" | "not_opened" | "clicked" | "not_clicked" | "replied" | "no_booking_days";
