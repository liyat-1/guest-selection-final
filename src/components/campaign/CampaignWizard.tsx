...
export function CampaignWizard() {
  ...
  const addFollowUp = () => setSteps((list) => renumber([...list, makeFollowUp(list.length - 1)]));
  ...
  if (minimized) {
    return (
      <div className="grid min-h-dvh place-items-end bg-zinc-900/70 p-4 font-sans">
        <div className="flex items-center gap-3 rounded-xl border border-zinc-200 bg-white px-4 py-3 shadow-pop">
  ...
  return (
    ...
    >
      <div
        ...
        className={`flex min-h-0 flex-1 flex-col overflow-hidden bg-zinc-50 shadow-pop ${
                ...
                <li key={s.id} className="flex min-w-0 flex-1">
                  <button
                    ...
                    className={`group flex min-w-0 flex-1 items-center gap-2.5 border-b-2 px-3 py-3 text-left transition-colors disabled:cursor-not-allowed disabled:opacity-40 ${
                      current ? "border-blue-600" : done ? "border-blue-200" : "border-transparent"
                    }`}
                  >
                    <span
      ...
      <TemplatePicker
        open={picker}
        currentId={
          pickerFor ? (steps.find((s) => s.id === pickerFor)?.email.templateId ?? null) : null
        }
        ...
        onApply={(id, next) => {
          ...
          patchStep(target, (s) => ({
            ...
            email: {
              ...
              body: s.email.body || next.body.paragraphs.map((p) => stripHtml(p.text)).join("\n\n"),
            },
          }));
          setPicker(false);
          setPickerFor(null);
      ...
      {toast && (
        <div className="pointer-events-none fixed bottom-6 left-1/2 z-[100] -translate-x-1/2 bg-zinc-900 px-4 py-2 text-[12.5px] font-medium text-white shadow-pop">
          {toast}
        </div>
      )}
    </div>
