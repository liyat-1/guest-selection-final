  ...
  return (
    <div className="flex flex-wrap items-end justify-between gap-3">
      <div>
        ...
        {subtitle ? (
          <p className="mt-1 max-w-2xl text-[13.5px] leading-relaxed text-slate-500">{subtitle}</p>
        ) : null}
      </div>
      {action}
    </div>
...
export function OtaShell({ children }: { children: ReactNode }) {
  ...
  const analytics = pathname === "/ota/analytics";

  return (
    <div className={`min-h-dvh ${analytics ? "bg-ota-canvas" : "bg-slate-50"} text-slate-900`}>
                ...
                <div className="absolute right-0 top-10 z-50 w-80 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-pop">
                  ...
                  <ul>
                    {NOTIFICATIONS.map((n) => (
                      <li
                        key={n.text}
                        className="flex gap-2.5 border-b border-slate-100 px-3 py-2.5 last:border-0"
                      >
                        <span
                          aria-hidden
                          className={`mt-1.5 size-1.5 shrink-0 rounded-full ${
                            n.tone === "issue" || n.tone === "recovery"
                              ? "bg-amber-500"
                              : "bg-blue-600"
                          }`}
                        />
