    ...
    <div ref={wrap} className="relative">
      <button
        ...
        className={`flex ${h} w-full items-center justify-between gap-2 rounded-lg border bg-white px-3 text-left font-medium text-zinc-900 transition-colors ${
          open ? "border-zinc-900 ring-2 ring-zinc-900/10" : "border-zinc-200 hover:border-zinc-300"
        }`}
      >
        <span className="min-w-0 flex-1 truncate">
          {renderValue
            ? renderValue(current)
            : (current?.label ?? <span className="text-zinc-400">{placeholder}</span>)}
        </span>
        <ChevronDown
          size={15}
      ...
      {open && (
        <div
          ...
          className={`absolute z-[80] mt-1.5 max-h-72 w-full min-w-[13rem] overflow-y-auto rounded-xl border border-zinc-200 bg-white p-1 shadow-pop shadow-zinc-900/10 ${
