...
export function RichTextEditor({
  ...
  const Toolbar = (compact = false) => (
    <>
      ...
      <button
        ...
        onMouseDown={run(() => exec("underline"))}
      >
        <Underline size={14} />
      </button>
      ...
      <button
        ...
        className={btn}
        onMouseDown={run(() => exec("insertOrderedList"))}
      >
        <ListOrdered size={14} />
      </button>
      <button
      ...
      {!compact && (
        <>
          ...
          <button
            ...
            className={btn}
            onMouseDown={run(() => exec("justifyLeft"))}
          >
            <AlignLeft size={14} />
          </button>
          <button
            ...
            className={btn}
            onMouseDown={run(() => exec("justifyCenter"))}
          >
            <AlignCenter size={14} />
          </button>
          <button
            ...
            className={btn}
            onMouseDown={run(() => exec("justifyRight"))}
          >
            <AlignRight size={14} />
          </button>
        </>
      ...
      <button
        ...
        className={btn}
        onMouseDown={run(() => exec("removeFormat"))}
      >
  ...
  const SwatchGrid = ({ kind }: { kind: "fore" | "hilite" }) => (
    <div className="absolute left-0 top-full z-[90] mt-1.5 w-[13.5rem] rounded-xl border border-zinc-200 bg-white p-2.5 shadow-pop shadow-zinc-900/10">
      ...
      <label className="mt-2.5 flex cursor-pointer items-center justify-between rounded-lg border border-zinc-200 px-2.5 py-1.5 text-[12px] text-zinc-600 hover:border-zinc-900">
        ...
        <input
          ...
          onChange={(e) => apply((el) => applyColor(el, kind, e.target.value))}
        />
      </label>
    </div>
  );
  ...
  return (
    <div className="space-y-2">
      {label && <span className="block text-[12px] font-medium text-zinc-600">{label}</span>}
      ...
      {bubble && (
        <div
          ...
          className="fixed z-[95] flex items-center gap-0.5 rounded-xl border border-zinc-200 bg-white px-1.5 py-1.5 shadow-pop shadow-zinc-900/20"
